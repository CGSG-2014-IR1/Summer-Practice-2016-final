function layout_light_img()
{
  this.Init = function( Ani )
  {
    ...
    var light_transparent_material = new THREE.ShaderMaterial(
      {
        uniforms:
        {
          ...
          "DiffuseColor": { type: "v3", value: new THREE.Vector3(0.5, 0.5, 0.5) },
          "DistortionStrength" : { type: "f", value: 0.05 }
        },
        side: THREE.DoubleSide,
        vertexShader: LoadShaderText("../js/shaders/light_transparent.vert"),
        fragmentShader: LoadShaderText("../js/shaders/light_transparent.frag"),
        transparent: true
      });

    var queen = CreateQueen(0.1, light_transparent_material);
    queen.Mesh.position.x = 0.15;
    queen.Mesh.position.z = -0.15;
    var king = CreateKing(0.1, light_transparent_material);
    king.Mesh.position.x = -0.15;
    king.Mesh.position.z = -0.15;

    var slayers = [];
    slayers.push(CreateSlayer(0.1, light_transparent_material));
    ...
    var mages = [];
    mages.push(CreateMage(0.1, light_transparent_material));
    ...
    var knights = [];
    knights.push(CreateKnight(0.1, light_transparent_material));
    ...
    {
      pawns.push(CreatePawn(0.1, light_transparent_material));
      pawns[pawns.length - 1].Mesh.position.x = i;
      pawns[pawns.length - 1].Mesh.position.z = 0.15;
    }

    var towers = [];
    towers.push(CreateTower(0.1, light_transparent_material));
    ...
    var base = CreatePlane(5, 5, new THREE.MeshPhysicalMaterial(
      {
        color: 0x333333
      }));
